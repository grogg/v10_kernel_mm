...
static unsigned int bcm2079x_dev_poll(struct file *filp, poll_table *wait)
{
    ...
    //ret = wait_event_interruptible(bcm2079x_dev->read_wq, mt_get_gpio_in(platform_data.irq_gpio));
    poll_wait(filp, &bcm2079x_dev->read_wq, wait);
    dev_info(&bcm2079x_dev->client->dev, "%s poll_wait after %d\n", __func__, mt_get_gpio_in(platform_data.irq_gpio));
#else
    //pr_info("%s poll_wait before %d\n", __func__, gpio_get_value(platform_data.irq_gpio));
    //ret = wait_event_interruptible(bcm2079x_dev->read_wq, gpio_get_value(platform_data.irq_gpio));
    poll_wait(filp, &bcm2079x_dev->read_wq, wait);
